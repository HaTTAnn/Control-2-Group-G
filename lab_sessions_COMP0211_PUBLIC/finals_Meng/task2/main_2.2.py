...
def main():
    ...
    # Constraints
    joint_min_bounds = np.array([-0.4, -0.4, -0.4])
    joint_max_bounds = np.array([0.4, 0.4, 0.4])
    large_number = 1e6
    ...
    plt.figure()
    for i in range(len(q_mes_all[0])):
        color = f'C{i}'  # Use matplotlib's color cycle
        plt.plot(time_all, np.array(q_mes_all)[:, i], color=color, label=f'Joint {i+1}')
        plt.plot(time_all, [goal_joints[i]]*len(time_all), color=color, 
                linestyle='--', label=f'Ref {i+1}')
    plt.title(f'Test 1 Joint Positions')
    plt.xlabel('Time [s]')
    plt.ylabel('Position [rad]')
    plt.legend()
    plt.grid(True)
    plt.savefig(f'test_1_positions.png')
    plt.close()
    ...
    plt.legend()
    plt.savefig('test_1_velocities.png')
    plt.close()
    ...
    plt.legend()
    plt.savefig('test_1_control_inputs.png')
    plt.close()
